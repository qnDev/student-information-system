...
public class ClassDAO {
    ...
    public MakeCourses addNewCourses(MakeCourses rg) {
        ...
        if (DBConnect.open()) {
            try {
                ...
                JOptionPane.showMessageDialog(this.classRegister, sqlMessage + "\n" + "Bạn không thể tạo mới được mã lớp:" + rg.getMa_lop_DK() + "!", "CTĐT Thông báo!", JOptionPane.INFORMATION_MESSAGE);
            } finally {
                DBConnect.close(ps);
            }
        }
     ...
     */
    public Register addNew(Register register) throws SQLException {
        PreparedStatement ps = null, ps1 = null;
        ResultSet rs;
        if (DBConnect.open()) {
            try {
                ps1 = DBConnect.cnn.prepareStatement("SELECT thoi_gian_bat_dau, thoi_gian_ket_thuc, thu FROM danh_sach_lop_mo_DK WHERE ma_lop_mo_DK =" + register.getMaLopDK());
                rs = ps1.executeQuery();
                if (rs.first()) {
                    Time starttime = rs.getTime("thoi_gian_bat_dau");
                    Time endtime = rs.getTime("thoi_gian_ket_thuc");
                    String thu = rs.getString("thu");
                    ps = DBConnect.cnn.prepareStatement("INSERT INTO danh_sach_SV_dang_ki_lop(id_sinh_vien_DK, ma_lop_mo_SV_DK, start_time, end_time, thu) VALUES (?, ?, ?, ?, ?)");
                    ps.setString(1, register.getMaSinhVienDK());
                    ps.setString(2, register.getMaLopDK());
    ...
    public void deleteCourses(String malop) {
